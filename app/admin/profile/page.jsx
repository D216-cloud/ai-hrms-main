...
export default function AdminProfilePage() {
  ...
  const handleSaveProfile = async () => {
    try {
      setSaving(true);
      let pictureUrl = profile.profileImage;
      // If profileImage is a data URL, upload it first
      if (pictureUrl && typeof pictureUrl === 'string' && pictureUrl.startsWith('data:')) {
        const up = await fetch('/api/hr/profile/upload', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ image: pictureUrl, filename: `hr_${Date.now()}.png` }),
        });
        const upBody = await up.json();
        if (!up.ok) {
          console.error('Upload failed:', upBody);
          throw new Error(upBody?.error || 'Failed to upload image');
        }
        pictureUrl = upBody.url;
      }
          ...
          <div className="flex items-start justify-between mb-6">
            <div className="flex items-start space-x-4">
              <div className="relative">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} title="Upload image" className="absolute -top-2 -left-2 bg-white dark:bg-slate-800 rounded-full p-1 shadow-md z-10">
                  <Upload className="w-4 h-4 text-slate-700 dark:text-slate-200" />
                </button>

                {profile.profileImage ? (
                  <img src={profile.profileImage} alt="profile" onClick={() => fileInputRef.current?.click()} className="w-20 h-20 rounded-full object-cover cursor-pointer shadow-md" />
                ) : (
                  <div onClick={() => fileInputRef.current?.click()} className="w-20 h-20 rounded-full bg-linear-to-br from-teal-400 to-cyan-500 flex items-center justify-center text-white text-2xl font-bold shadow-md cursor-pointer">{getInitials(profile.fullName)}</div>
                )}
              </div>
              <div>
            ...
            <div className="flex items-center space-x-3">
              <Link href="/admin/interview">
                <button className="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition">Interview</button>
              </Link>
              <Link href="/admin/message">
                <button className="px-4 py-2 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-600 transition">Message</button>
              </Link>
          ...
          {editMode.profile ? (
            <div className="space-y-4 bg-slate-50 dark:bg-slate-700 p-4 rounded-lg">
              ...
              <div>
                ...
                {profile.profileImage && (<p className="text-sm text-slate-600 mt-2">Image selected</p>)}
              </div>
              <div>
