...
export default function JobSeekerDashboard() {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      // Fetch saved jobs
      const savedRes = await fetch("/api/seeker/saved-jobs");
      let savedData = { savedJobs: [] };
      
      if (savedRes.ok) {
        savedData = await savedRes.json();
        console.log("Saved jobs response:", savedData);
      } else {
        console.warn("Failed to fetch saved jobs:", savedRes.status);
      }
      
      setSavedJobs(Array.isArray(savedData.savedJobs) ? savedData.savedJobs : []);
      ...
      setStats({
        ...
        interviews: 0,
        saved: Array.isArray(savedData.savedJobs) ? savedData.savedJobs.length : 0,
      });
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-md hover:shadow-lg transition-all duration-300 group">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-4xl font-bold text-slate-900 dark:text-white mt-2">{stats.saved}</p>
              </div>
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-500 mt-3 font-semibold">+3 this month</p>
          </div>
