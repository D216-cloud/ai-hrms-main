...
-- This migration creates the job_seeker_profiles table if it doesn't exist

-- Create the job_seeker_profiles table
...
CREATE INDEX IF NOT EXISTS idx_job_seeker_profiles_email ON job_seeker_profiles(email);

-- Enable RLS (Row Level Security)
ALTER TABLE job_seeker_profiles ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Users can only read/write their own profile
CREATE POLICY IF NOT EXISTS job_seeker_profiles_select_own 
  ON job_seeker_profiles 
  FOR SELECT 
  USING (auth.uid()::text = email OR email IS NOT NULL);

CREATE POLICY IF NOT EXISTS job_seeker_profiles_insert_own 
  ON job_seeker_profiles 
  FOR INSERT 
  WITH CHECK (auth.uid()::text = email);

CREATE POLICY IF NOT EXISTS job_seeker_profiles_update_own 
  ON job_seeker_profiles 
  FOR UPDATE 
  USING (auth.uid()::text = email);

CREATE POLICY IF NOT EXISTS job_seeker_profiles_delete_own 
  ON job_seeker_profiles 
  FOR DELETE 
  USING (auth.uid()::text = email);
