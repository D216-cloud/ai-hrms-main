...
export const authOptions = {
  ...
  jwt: {
    // Set to true if using issued/expiration times
    maxAge: 30 * 24 * 60 * 60, // 30 days
  },
  // Ensure cookies work properly across subdomains
  // Use secure cookies only when NEXTAUTH_URL is HTTPS to avoid cookies not being sent
  // (common issue when NODE_ENV=production but app is behind HTTP during some deploy previews)
  // When secure cookies are enabled, prefix cookie names with __Secure- for better browser behavior.
  cookies: (() => {
    const useSecure = !!(process.env.NEXTAUTH_URL && process.env.NEXTAUTH_URL.startsWith("https"));
