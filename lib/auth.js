      ...
      if (user) {
        ...
        if (account?.provider === "google") {
          ...
          if (fetchError || !existingUser) {
            ...
            // Default to 'job_seeker' but promote to 'hr' for configured HR domains.
            let role = "job_seeker"; // Default to job seeker for Google users
            try {
              const hrDomainsEnv = process.env.HR_EMAIL_DOMAINS || "";
              const hrDomains = hrDomainsEnv.split(",").map(s => s.trim().toLowerCase()).filter(Boolean);
              const emailDomain = (user.email || "").split("@")[1]?.toLowerCase();
              if (emailDomain && hrDomains.includes(emailDomain)) {
                role = "hr";
              }
            } catch (err) {
              // If anything goes wrong, fall back to job_seeker
              role = "job_seeker";
            }
