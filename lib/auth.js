  ...
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        // If the login is for a job seeker, authenticate against job_seekers table
        if (credentials.role === 'job_seeker') {
          // Look up job seeker by email
          let { data: seeker, error: seekerErr } = await supabaseAdmin
            .from('job_seekers')
            .select('*')
          ...
          // If not found, create job_seeker entry (auto-signup behavior)
          if (seekerErr || !seeker) {
            ...
            if (createErr) {
              throw new Error('Failed to create job seeker account');
            }
            seeker = created;
          } else {
            // verify password
            const isValid = await bcrypt.compare(credentials.password, seeker.password_hash || '');
            if (!isValid) {
          ...
          return {
            id: seeker.auth_id || seeker.id,
            email: seeker.email,
            name: seeker.full_name || seeker.email.split('@')[0],
            role: 'job_seeker',
          };
        }

        // Fallback: credential users for HR continue to use hr_users table
        ...
        if (fetchError || !user) {
          ...
          // Verify password for existing HR user
