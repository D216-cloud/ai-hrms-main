...
// Warn about common misconfiguration in production
if (process.env.NODE_ENV === 'production') {
  if (!process.env.NEXTAUTH_URL || !process.env.NEXTAUTH_URL.startsWith('https://')) {
    console.warn('NEXTAUTH_URL is not set to an https:// URL. Set NEXTAUTH_URL=https://<your-domain> in Render.');
  }
  if (!process.env.NEXTAUTH_SECRET) {
    console.warn('NEXTAUTH_SECRET is not set. NextAuth tokens and cookies will be invalid.');
  }
}
  ...
  cookies: {
    sessionToken: {
      ...
      options: {
        ...
        // Optionally set an explicit cookie domain in production if your deployment
        // requires it (e.g. use `SESSION_COOKIE_DOMAIN=ai-hrms-hu16.onrender.com` in Render env).
        // If not set, NextAuth will set a cookie for the request host by default.
